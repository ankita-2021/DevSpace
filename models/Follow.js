...
Follow.prototype.validate = async function (action) {
  ...
  let doesFollowAlreadyExist = await followsCollection.findOne({
    followedId: this.follwedId,
    authorId: new ObjectID(this.authorId),
  });
  if (action == "follow") {
    if (doesFollowAlreadyExist) {
      this.errors.push("You are already following");
    }
  }
  if (action == "unfollow") {
    if (!doesFollowAlreadyExist) {
      this.errors.push("You are already not following");
    }
  }

  if (this.follwedId.equals(this.authorId)) {
